...
using Expr = variant<nse::Ptr<class Val_expr>, nse::Ptr<class Bin_expr>>;
...
public:
    ...
    double expressionCalc(const Expression& expr) const;
...
class Val_expr {
public:
    string repr() const { return to_string(val); }
...
double expressionCalc()
{
    auto calc_v = [](const auto& x) { return this->calc(); };
    return visit(calc_v, expr.expr);
}

string Bin_expr::repr() const
{
...
public:
    ...
    explicit Token_stream(istream& is): input{is} {}
private:
    istream& input;
