...
def draw_textured_floor(texture_id):
    ...
    glDisable(GL_LIGHTING)  # Disable lighting for the floor
    ...
    glEnable(GL_LIGHTING)  # Re-enable lighting after drawing the floor
...
def setup_lighting():
    ...
    glEnable(GL_LIGHT0)
    glLightfv(GL_LIGHT0, GL_POSITION, light_pos)  # Light position (dynamic)
    glLightfv(GL_LIGHT0, GL_DIFFUSE, [1, 1, 1, 1])  # White diffuse light
    glLightfv(GL_LIGHT0, GL_AMBIENT, [0.2, 0.2, 0.2, 1])  # Soft ambient light
    glLightfv(GL_LIGHT0, GL_SPECULAR, [1, 1, 1, 1])  # Specular highlights
