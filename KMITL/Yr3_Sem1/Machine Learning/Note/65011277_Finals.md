...
#### Example Calculations for Each Class
...
pandas: For data manipulation (not used directly in this example but often useful).
librosa: For audio processing and feature extraction.
sklearn: For machine learning tasks such as model training and evaluation.
Load the Dataset:

The load_data function iterates through the files in the specified directory, loading each WAV file.
The label is extracted from the filename (assuming the label is the first character).
MFCC features are computed using librosa.feature.mfcc() and averaged over time to create a single feature vector per audio file.
Preprocess the Data:
